    ...
    private  List<ContactData> generateContacts(int count) {
        ...
        for (int i=0;i< count;i++) {
            contacts.add(new ContactData().withFirstname(String.format("First name %s", i))
                    .withMiddlename(String.format("Middle name %s", i)).withLastname(String.format("Last name %s", i)).withEmail(String.format("email%s@test.com",i))
                    .withGroup(String.format("new")).withBirthYear(String.format("199%s",i)).withAnniversaryYear(String.format("202%s",i)));
        }
